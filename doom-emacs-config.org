...
- [[https://github.com/doomemacs/doomemacs/issues/241][Understanding how to efficiently setup packages. Doom #241]]
...
*** Font
Use [[https://github.com/jonz94/Sarasa-Gothic-Nerd-Fonts][Nerd-Font-patched]]
[[https://github.com/be5invis/Sarasa-Gothic][Sarasa]] font.
Slab variant is serif. No ligature, half-width dash as in =Fixed=.
#+begin_src emacs-lisp :tangle lisp/config-ui.el :mkdirp yes
...
[[https://github.com/rougier/svg-tag-mode/tree/main][svg-tag-mode]]
is a minor mode to replace keywords or regular expression with SVG tags.
...
Add keyword mapping, adapted from the example-2.
#+begin_src emacs-lisp :tangle lisp/config-ui.el :mkdirp yes
...
#+begin_src emacs-lisp :tangle lisp/config-completion.el :mkdirp yes
...
# Disable writegood-mode when in org source code block.
# Reference in [[https://emacs.stackexchange.com/questions/55944/disable-minor-mode-inside-source-blocks][this]] SO link
# #+begin_src emacs-lisp :tangle lisp/config-checker.el :mkdirp yes
# (use-package! writegood-mode
#   :config
#   (defun my/hook-disable-writegood-mode-in-org-source-blocks ()
#     "Disable writegood-mode in source code blocks of org"
#     (add-hook '(lambda () (interactive) (setq writegood-mode nil)) 'org-in-src-block-p nil :local))
#   (add-hook 'org-mode-hook 'my/hook-disable-writegood-mode-in-org-source-blocks)
# )
# #+end_src
...
*** org-bar to show better indenting

[[https://github.com/tonyaldon/org-bars][tonyaldon/org-bars]] is not available on MELPA
and needs to be cloned from GitHub.
...
#+begin_src emacs-lisp :tangle lisp/config-org-bars.el :mkdirp yes
...
TODO:
- blinking when opening magit status and switching to modified buffer from there
- lines containing CJK characters do not have continuous bar lines. Seems to related to the height of CJK line.
#+begin_src emacs-lisp :tangle config.el :mkdirp yes
(require 'config-org-bars)
#+end_src

*** Drill (deprecated in usage)
...
The gnuplot package pinned in the org module is a bit old, unpin it here.
#+begin_src emacs-lisp :tangle packages.el :mkdirp yes
(unpin! gnuplot)
(unpin! gnuplot-mode)
#+end_src
