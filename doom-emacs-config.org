...
(defvar my/bibtex-file
  (expand-file-name "bibliography.bib" "~/database")
...
(map! :leader
      (:prefix-map ("e" . "extra-my")
        (:prefix-map ("i" . "input")
         ...
         :desc "Tex"             "c"  '(lambda () (interactive) (set-input-method "TeX"))
)))
#+end_src
...
(use-package! pyim
  ...
  ;; 加入个人词库，包括发音和词频。
  ;; export.pyim 和 count-export.pyim 是从 pyim 导出的文件，避免重装系统或重置 emacs.d 后丢失。
  (let ((pdict  "dict/export.pyim")
        (pcount "dict/count-export.pyim"))
    (add-to-list 'pyim-dicts
        `(:name "个人导出"
          :file ,(concat doom-private-dir pdict)))
    (pyim-import-words-and-counts pcount))
...
(use-package! org
  ...
  (setq org-capture-templates
        `(
          ...
          ("j" "Journal Log" plain
               (file+function "journal/log.org" my/org-goto-today-node)
               "%<%H:%M> %?")
...
(use-package! bibtex-completion
  :config
  (setq bibtex-completion-notes-path (expand-file-name "ref" my/org-dir)
...
(use-package citar
  ...
  (citar-notes-paths (list (expand-file-name "ref" my/org-dir)))
)
#+end_src
