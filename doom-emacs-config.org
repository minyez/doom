...
;                                        (svg-progress-count (substring tag 1 -1)))))
      ...
      (" \\(WAIT\\) " . ((lambda (tag) (svg-tag-make "WAIT" :face '+org-todo-on-hold :inverse t :margin 0))))
      (" \\(HOLD\\) " . ((lambda (tag) (svg-tag-make "HOLD" :face '+org-todo-on-hold :inverse t :margin 0))))
      (" \\(CANCELLED\\) " . ((lambda (tag) (svg-tag-make "CANCELLED" :face '+org-todo-cancel :inverse t :margin 0))))
      ...
      ;; Inactive date  (with or without day name, with or without time)
       ...
       (,(format "\\(\\[%s \\)%s\\]" date-re day-time-re) .
        ((lambda (tag)
           (svg-tag-make tag :beg 1 :inverse nil :crop-right t :margin 1 :face 'org-date))))
       (,(format "\\[%s \\(%s\\]\\)" date-re day-time-re) .
        ((lambda (tag)
